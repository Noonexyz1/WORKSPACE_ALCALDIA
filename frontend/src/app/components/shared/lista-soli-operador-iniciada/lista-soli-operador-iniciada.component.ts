...
import { RowTablePendienteOperadorComponent } from './row-table-operador-iniciada/row-table-operador-iniciada.component';
...
@Component({
  ...
  templateUrl: './lista-soli-operador-iniciada.component.html',
  styleUrl: './lista-soli-operador-iniciada.component.css'
})
export class ListaSoliIniciadaOperadorComponent {
  ...
  listarSolicitudes(): void {
    const url = 'http://localhost:8081/operador/verSolicitudesIniciadas';
    ...
    this.http.post<SolicitudOperaResponse[]>(url, body).pipe(
      ...
      catchError(error => {
        ...
        alert('Hubo un error al listar las solicitudes iniciadas para el operador');
